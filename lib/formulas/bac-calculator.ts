...
import { calculateBAC, timeToZeroBAC, peakBAC } from "@/lib/calculators/bac";
...
export const bacCalculatorFormula: ICalculatorFormula = {
  ...
  calculate(inputs) {
    ...
    const bac = calculateBAC(inputs.drinks, inputs.weightKg, isMale, inputs.hours);
    const zeroTime = timeToZeroBAC(inputs.drinks, inputs.weightKg, isMale);
    const peak = peakBAC(inputs.drinks, inputs.weightKg, isMale);
    ...
    return {
      ...
      secondary: [
        { label: "Peak BAC (if all consumed at once)", value: round(peak, "generic", 3), unit: "%" },
        ...
        { label: "Standard drinks consumed", value: Math.round(inputs.drinks), unit: `(14 g alcohol each)` },
      ...
      interpretation:
        ...
        `Peak BAC would be approximately ${round(peak, "generic", 3)}%. ` +
